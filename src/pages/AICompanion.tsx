...
export default function AICompanion() {
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    // All messages now go through the standard flow to allow the AI to handle casual chat naturally
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className={`w-full min-h-[52px] max-h-32 px-4 py-3 bg-muted rounded-xl border-0 resize-none focus:outline-none focus:ring-2 focus:ring-primary/30 text-foreground placeholder:text-muted-foreground transition-opacity ${isDisabled ? "opacity-50 cursor-not-allowed" : ""
                      }`}
                    rows={1}
                  />
                </div>
